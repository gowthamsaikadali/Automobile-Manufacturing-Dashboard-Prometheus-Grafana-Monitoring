...
# ---------------------------------------------------------------------------
# EBS CSI driver - required for ANY PersistentVolumeClaim on this cluster.
# EKS does not ship this by default (the in-tree EBS provisioner was
# removed years ago) - without it, Prometheus/Grafana/Loki's PVCs in Part B
# would stay stuck in Pending forever with no obvious error.
# ---------------------------------------------------------------------------
module "ebs_csi_irsa_role" {
  source  = "terraform-aws-modules/iam/aws//modules/iam-role-for-service-accounts-eks"
  version = "~> 5.44"

  role_name = "${var.project_name}-${var.environment}-ebs-csi-role"
