...
This is a clean, from-scratch build: Flask app (app tier) + MySQL RDS
(data tier), running on your existing EKS cluster, behind an AWS ALB —
**no domain name needed**, you just use the ALB's auto-generated DNS
name, exactly like your screenshot.

Everything below assumes: VPC, EKS cluster, and RDS instance already
exist from your earlier Terraform work. If they don't, provision those
first (RDS in private subnets, EKS with a node group, AWS Load Balancer
Controller installed) — say the word and I'll give you that Terraform
too, but it's a separate concern from the app + monitoring you asked
for here.
...
aws ecr create-repository --repository-name automobile-app --region ap-south-1

# Authenticate, tag, push
aws ecr get-login-password --region ap-south-1 | \
  docker login --username AWS --password-stdin <ACCOUNT_ID>.dkr.ecr.ap-south-1.amazonaws.com

docker tag automobile-app:latest <ACCOUNT_ID>.dkr.ecr.ap-south-1.amazonaws.com/automobile-app:latest
docker push <ACCOUNT_ID>.dkr.ecr.ap-south-1.amazonaws.com/automobile-app:latest
```

### Step 3: Fill in real secrets
Edit `k8s/service.yaml`, replace the `Secret` block's placeholder values
with your real RDS endpoint/credentials, and generate a real
`SECRET_KEY`:
```bash
