...
First, generate a real password hash and paste it into `app\schema.sql`
in place of the placeholder (`python` must be on your PATH — Python
from python.org or the Microsoft Store both work):
```powershell
python -c "import bcrypt; print(bcrypt.hashpw(b'YourStrongPassword', bcrypt.gensalt()).decode())"
```
Then load the schema (requires the MySQL client — `mysql.exe`, from
MySQL Workbench or the MySQL Community Server installer):
```powershell
mysql -h $dbCreds.host -u $dbCreds.username -p automobile_db < app\schema.sql
```
PowerShell's `<` redirection for external commands works for this since
`mysql.exe` is a native binary; if it complains, use:
```powershell
Get-Content app\schema.sql | mysql -h $dbCreds.host -u $dbCreds.username -p automobile_db
```
