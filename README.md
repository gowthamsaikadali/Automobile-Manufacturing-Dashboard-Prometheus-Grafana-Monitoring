...
```
Keep that JSON handy — you'll need `username`, `password`, and `host`
for Steps 1 and 3 below.
...
```bash
# host/username/password come from Step 0.3 above
...
with the real `host`/`username`/`password` from the Secrets Manager JSON
you fetched in Step 0.3, and generate a real `SECRET_KEY`:
```bash
